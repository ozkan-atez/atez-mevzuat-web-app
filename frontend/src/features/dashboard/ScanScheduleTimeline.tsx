...
export function ScanScheduleTimeline() {
    ...
    <div className="pt-6 border-t border-slate-800/80 mt-2 space-y-5 relative z-10" data-purpose="live-cron-process-bar">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3.5">
          <div className={`relative flex items-center justify-center w-8 h-8 rounded-full border-2 ${active ? 'border-indigo-400/30 text-indigo-400' : 'border-slate-700 text-slate-500'}`}>
            {active && (
      ...
      <div className="relative px-2 pt-2">
        <div className="relative flex items-center justify-between mb-4">
          <div className="absolute left-4 right-4 top-1/2 -translate-y-1/2 h-[3px] bg-slate-800 rounded-full z-0" />
          <div
            className="absolute left-4 top-1/2 -translate-y-1/2 h-[3px] bg-indigo-500 rounded-full z-0 shadow-sm shadow-indigo-500/50 transition-[width] duration-500"
            style={{ width: `${progressWidth(slots)}%` }}
          />
...
function progressWidth(slots: ScheduleSlot[]): number {
  if (slots.length < 2) return 0
  const lastActive = slots.reduce((last, slot, index) => slot.state === 'COMPLETED' || slot.state === 'RUNNING' || slot.state === 'ATTENTION' ? index : last, -1)
  return lastActive < 0 ? 0 : (lastActive / (slots.length - 1)) * 92
}
